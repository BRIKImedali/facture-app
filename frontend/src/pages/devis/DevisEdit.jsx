  ...
  useEffect(() => {
    Promise.all([
      ...
      if (d.statut !== 'BROUILLON' && d.statut !== 'ENVOYE') {
        toast.error(`Ce devis (${d.statut}) n'est plus modifiable.`);
        navigate(`/devis/${id}`);
        return;
      }
      setDateExpiration(d.dateExpiration
        ? d.dateExpiration.split('/').reverse().join('-')   // dd/MM/yyyy → yyyy-MM-dd
        : '');
      setNotes(d.notes || '');
      setRemise(d.remise || 0);
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      <div className="page-header">
        <h1 className="page-title">✏️ Modifier le devis</h1>
        <Link to={`/devis/${id}`} className="btn btn-secondary">← Retour</Link>
      </div>
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div className="form-grid">
            ...
            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
              ...
              <textarea value={notes} onChange={e => setNotes(e.target.value)} className="form-control" rows={2}
                placeholder="Ex: Validité 30 jours..." />
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          ...
          <Link to={`/devis/${id}`} className="btn btn-secondary">Annuler</Link>
        </div>
      </form>
    </div>
  );
